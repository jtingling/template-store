...
export default function ContentHeader({image, productType, blurhash }) {
    ...
    return (
        <div className='position-relative mt-5'>
            <BlurhashHero width={width} height={(height * .50)} hash={blurhash} image={image} loadImg={loadImg} setHero={setHero} hero={hero}/>
            <TitleCard title={productType} description={productDescription.data} loadImg={loadImg} hero={hero}/>
        </div>
    )
}